...
我们要写些数据到数据库。所以 `@Component` 只是一个普通的 Spring Bean，在这里注入 `reservationRepository` 注入到构造器。我需要 `@RequireArgsConstructor`
还有参数 定义这些数据
然后我将数据写入到数据库
通过创建一些名字 好吗？
所以我要过一遍
